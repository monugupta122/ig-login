...
class Instagram_api {
    ...
    protected $_api_urls = array(
    	'user'						=> 'https://graph.instagram.com/%s&access_token=%s',
     ...
     */
    public function get_user($user_id, $token) {
    	
    	$user_request_url = sprintf($this->_api_urls['user'], $user_id, $token);
    ...
    public function user_media_liked(){

    	$user_media_liked_request_url = sprintf($this->_api_urls['user_recent'], $user_id, $this->access_token, $count, $max_id, $min_id, $max_timestamp, $min_timestamp);

    	return $this->__apiCall($user_media_liked_request_url);
